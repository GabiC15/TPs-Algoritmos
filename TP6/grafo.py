...
class Grafo():
    ...
    def camino(self, resultados, destino):
    ...
    def contar_maravillas(self):
        ...
        aux = self.__inicio
        while aux is not None:
            for pais in aux.datos['paises']:
                if pais not in paisesRes:
                    paisesRes[pais] = {'arq': 0, 'nat': 0}
                if aux.datos['tipo'] == 'natural':
                    paisesRes[pais]['nat'] += 1
                else:
                    paisesRes[pais]['arq'] += 1
            aux = aux.sig
...
g.insertar_vertice(
    'L', datos={'tipo': 'n', 'pais': 'argentina-brasil-paragauy'})
